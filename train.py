...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer, lr_scheduler, args):
    ...
    for e in range(1, args.n_epoch+1):
        ...
        if best_acc < test_acc:
            best_acc = test_acc
            stop = 0
            # Save the model if best accuracy is improved
            torch.save(model.state_dict(), 'source-only_{}_{}.pth'.format(args.src_domain,args.tgt_domain))
            print('Saved new best model with accuracy: {:.4f}'.format(test_acc))
